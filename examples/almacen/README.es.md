...
> Quiver es una librería: el paquete instalable expone **solo la API**. El panel de administración / portal (la SPA) se ejecuta por separado desde el directorio `frontend/` del repositorio. El backend no sirve la UI automáticamente.
...
Desde PyPI (instalación pública):

```bash
pip install fastapi-quiver
```

Para desarrollar contra el código del repo, usa la instalación editable local:
...
### 6. Arrancar el backend (la API)

```bash
uvicorn main:app --reload
```

La API se sirve bajo `QUIVER_PREFIX` (por defecto `/quiver/v1`).

### 7. Arrancar el frontend (en otra terminal)

La SPA de administración/portal vive en el directorio `frontend/` del repositorio y se ejecuta aparte:
...
Repositorio: https://github.com/rnkr69/quiver
