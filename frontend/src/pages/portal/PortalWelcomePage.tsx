  ...
  if (!isDev) {
    return (
      <div className="text-center py-20 px-6">
        <QuiverLogo size={52} />
        <h1 className="text-3xl font-semibold text-gray-900 mt-4 mb-2">{t('portal.welcome.title')}</h1>
        <p className="text-base text-gray-600 mb-6">
          {data?.message ?? t('portal.welcome.comingSoon')}
        </p>
        <a
  ...
  return (
    <div className="max-w-[640px] mx-auto">
      <div className="bg-brand-50 border border-brand-100 rounded px-3.5 py-2.5 mb-8 flex gap-2.5 items-start text-md text-brand-700">
        ...
        <div>
          <strong>{t('portal.welcome.devNotice.title')}</strong>
          {' '}{t('portal.welcome.devNotice.editPrefix')} <code className="bg-brand-100 px-1 py-px rounded text-sm">src/pages/portal/PortalWelcomePage.tsx</code> {t('portal.welcome.devNotice.editSuffix')}
        </div>
      </div>

      <div className="text-center mb-8">
        <QuiverLogo size={52} />
        <h1 className="text-3xl font-semibold text-gray-900 mt-4 mb-2">{t('portal.welcome.active')}</h1>
        <p className="text-base text-gray-600">{t('portal.welcome.userZone')}</p>
      </div>

      <Card className="p-5 max-w-[420px] mx-auto mb-6 font-mono text-md">
        <div className="flex flex-col gap-2.5 text-gray-800">
          <div className="flex gap-2 items-center">
            <span className="text-gray-500 min-w-[80px]">{t('portal.welcome.info.user')}:</span>
            <span>{user?.email ?? '—'}</span>
          </div>
          <div className="flex gap-2 items-start">
            <span className="text-gray-500 min-w-[80px]">{t('portal.welcome.info.roles')}:</span>
            <div className="flex flex-wrap gap-1">
              {user?.roles.map(r => <Badge key={r} variant="client">{r}</Badge>)}
            </div>
          </div>
          <div className="flex gap-2">
            <span className="text-gray-500 min-w-[80px]">{t('portal.welcome.info.environment')}:</span>
            <span>{data?.env ?? 'development'}</span>
          </div>
          <div className="flex gap-2">
            <span className="text-gray-500 min-w-[80px]">{t('portal.welcome.info.version')}:</span>
